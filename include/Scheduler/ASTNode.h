...
public :
    ...
    void evaluate() override {
        ...
        if (left_) left_->evaluate();
        if (right_) right_->evaluate();
        operation_->execute();
        evaluated_ = true;
    }
    ...
    void print(int depth = 0) const override {
        ...
        std::cout << indent << "Operation: " << (type_ == OperationType::Add ? "ADD" : type_ == OperationType::Multiply? "Multiply" : "Assignment") << std::endl;
