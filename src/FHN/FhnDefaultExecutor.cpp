...
bool FhnDefaultExecutor::decompose(FhnBackendCtx *ctx,
                                   ...
                                   FhnBuffer **buffers) {
    auto call = [&](FhnOpCode op, FhnBuffer *res,
                    const FhnBuffer *const *ops,
                    const int64_t *params, const double *fparams) -> int {
        auto it = dispatch_.find(static_cast<int>(op));
        if (it == dispatch_.end()) return -1;
    ...
    case FHN_HMULT: {
        ...
        const FhnBuffer *ops[] = {
            buffers[inst.operands[0]], buffers[inst.operands[1]], nullptr, nullptr
        };
    ...
    case FHN_HROT: {
        if (!dispatch_.count(static_cast<int>(FHN_MULT_KEY)) ||
            !dispatch_.count(static_cast<int>(FHN_ROTATE)))
            return false;
        const FhnBuffer *ops[] = {
    ...
    case FHN_HROT_ADD: {
        ...
        const FhnBuffer *add_ops[] = {
            buffers[inst.result_id], buffers[inst.operands[1]], nullptr, nullptr
        };
        return call(FHN_ADD_CC, buffers[inst.result_id], add_ops, inst.params, inst.fparams) == 0;
    }
    case FHN_HCONJ_ADD: {
        if (!dispatch_.count(static_cast<int>(FHN_CONJUGATE)) ||
            !dispatch_.count(static_cast<int>(FHN_ADD_CC)))
            return false;
        const FhnBuffer *ops[] = {
            buffers[inst.operands[0]], nullptr, nullptr, nullptr
        };
    ...
    case FHN_MAD: {
        if (!dispatch_.count(static_cast<int>(FHN_MULT_CS)) ||
            !dispatch_.count(static_cast<int>(FHN_ADD_CC)))
            return false;
