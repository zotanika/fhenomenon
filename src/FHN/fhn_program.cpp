...
FhnProgram *fhn_program_alloc(uint32_t num_instructions,
    ...
    auto *program =
        static_cast<FhnProgram *>(std::calloc(1, sizeof(FhnProgram)));
    if (program == nullptr) {
        return nullptr;
    }
    ...
    if (num_instructions > 0) {
        program->instructions = static_cast<FhnInstruction *>(
            std::calloc(num_instructions, sizeof(FhnInstruction)));
    ...
    if (num_inputs > 0) {
        program->input_ids =
            static_cast<uint32_t *>(std::calloc(num_inputs, sizeof(uint32_t)));
    ...
    if (num_outputs > 0) {
        program->output_ids = static_cast<uint32_t *>(
            std::calloc(num_outputs, sizeof(uint32_t)));
