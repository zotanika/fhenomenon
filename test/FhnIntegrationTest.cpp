...
TEST_F(FhnIntegrationTest, OperationsToASTToFhnProgramToExecution) {
  ...
  auto root = std::make_shared<OperatorNode<int>>(add_op, OperationType::Add, leaf_a, leaf_b, result);
  ...
  for (uint32_t i = 0; i < prog->num_inputs; i++)
    if (prog->input_ids[i] > max_id)
      max_id = prog->input_ids[i];
  for (uint32_t i = 0; i < prog->num_instructions; i++)
    if (prog->instructions[i].result_id > max_id)
      max_id = prog->instructions[i].result_id;
  ...
  // 6. Encrypt inputs into buffers through the host-side data plane
  // input 0 = a (value 10), input 1 = b (value 20)
  for (uint32_t i = 0; i < prog->num_inputs; i++) {
    int err = toyfhe_fhn_encrypt_i64(ctx_, buffers[prog->input_ids[i]], (i == 0) ? 10 : 20);
    ASSERT_EQ(err, 0);
  }

  // 7. Execute the (compute-only) program
  int err = executor_->execute(ctx_, prog, buffers.data());
  EXPECT_EQ(err, 0);

  // 8. Decrypt the output buffer host-side
  int64_t decrypted = 0;
  err = toyfhe_fhn_decrypt_i64(ctx_, buffers[prog->output_ids[0]], &decrypted);
  EXPECT_EQ(err, 0);
...
TEST_F(FhnIntegrationTest, ChainedOperations) {
  ...
  auto leaf_a = std::make_shared<OperandNode<int>>(a);
  auto leaf_b = std::make_shared<OperandNode<int>>(b);
  auto node1 = std::make_shared<OperatorNode<int>>(add1, OperationType::Add, leaf_a, leaf_b, t);
  auto node2 = std::make_shared<OperatorNode<int>>(add2, OperationType::Add, node1, leaf_a, result);
  ...
  // Encrypt inputs host-side: input_ids correspond to leaf_a and leaf_b.
  // Since leaf_a is shared (used twice), there are 2 unique inputs.
  // We know a=5, b=3
  int values[] = {5, 3};
  for (uint32_t i = 0; i < prog->num_inputs; i++) {
    ASSERT_EQ(toyfhe_fhn_encrypt_i64(ctx_, buffers[prog->input_ids[i]], values[i]), 0);
  }

  // Execute the compute-only program
  int err = executor_->execute(ctx_, prog, buffers.data());
  EXPECT_EQ(err, 0);

  // Decrypt the output buffer host-side
  int64_t result_val = 0;
  EXPECT_EQ(toyfhe_fhn_decrypt_i64(ctx_, buffers[prog->output_ids[0]], &result_val), 0);
